defmodule Rumbl.AccountsTest do
  ...
  # Describe blocks allow us to apply different test setups to a whole block of
  # tests. For example, each of our credentials tests will require a user.
  describe "authenticate_by_email_and_pass/2" do
    @email "user@localhost"
    @pass "123456"

    # user_fixure is from Rumbl.TestHelper which creates a user in the DB and returns that user
    # here we pass our email and password, and it uses Enum.into to create the user
    #  (Enum.into doesn't override keys already present)
    setup do
